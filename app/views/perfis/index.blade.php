...
@section('content')
	<h4>
		<span class="fa fa-list-alt fa-2x"></span> Listagem de Perfis
		<a href="{{ URL::to('perfil/create') }}" class="btn btn-primary navbar-right"><span class="glyphicon glyphicon-plus-sign"></span> Novo</a>
	</h4>
	<hr>
	{{ Form::open(array('url' => 'perfil', 'method' => 'get', 'class' => 'form-inline', 'role' => 'form')) }}
		<div class="form-group">
			{{ Form::text('descricao', $descricao, array('placeholder' => 'Descricao', 'class' => 'form-control')) }}
		</div>
		{{ Form::button('<span class="fa fa-search"></span> Pesquisar', array('type' => 'submit', 'class' => 'btn btn-default')) }}
	{{ Form::close() }}
	<hr>
	@if($perfis->getItems())
		...
		<table class="table table-striped table-hover">
			<thead>
				<tr>
					...
					<th><a href="{{ URL::to('perfil?sort=updated_at' . $str) }}">Alterado</a></th>
					<th colspan="2"></th>
				</tr>
			</thead>
			<tbody>
				@foreach ($perfis as $perfil)
					<tr>
						...
						<td class="action">{{ link_to('perfil/' . $perfil->id . '/edit', 'Editar', array('class' => 'btn btn-warning btn-sm', 'title' => 'Editar')) }}</td>
						<td class="action">
